...
1. **`HomeController.java`**: Add a rest controller at `src/main/java/com/example/my_maven_app/`.
...
2. **Create React App**: Use Create React App to scaffold the application.
    
```bash
npx create-react-app .
```

3. **Customize App**: Edit `src/App.js` to add custom content (e.g., a welcome message).
...
5. **`Dockerfile`**: Configure the container image using Nginx to serve the built app.
...
```
   ...
   - Maven Example: `docker service create --name my-maven-app --publish 8080:8080 myuser/my_maven_app:latest`
   - React Example: `docker service create --name my-react-app --publish 80:80 myuser/my_react_app:latest`
...
- **No Access**: Ensure Swarm is initialized (`docker node ls`) and the service is running. Access directly at `localhost:<host_port>`.
- **Image Issues**: Confirm Docker Hub authentication and image availability.
- **Port Conflicts**: Check if the host port is free with `netstat -tlnp | grep <port>`.
- **Service Logs**: Use `docker service logs <app-name>` for debugging container issues.
